...
4. **Subprocesses get secrets via env injection at spawn, scoped to need.** The canonical helper is `backend/services/engine_env.py:build_engine_env()` — it resolves `HF_TOKEN` (plus the child's expected alias, e.g. `YOUR_HF_TOKEN`) from the resolver cascade; `sonitranslate.py`'s `start()` is the reference caller. Launchers with no HF need (ffmpeg, `open`/`explorer`/`xdg-open`) get nothing. Never inject into a spawn "just in case."
5. **Anything leaving the machine is default-deny.** The bug reporter (`frontend/src/components/ReportBugButton.jsx`) builds its body by explicitly pushing known fields (platform, device, versions) — it never serializes an object wholesale, so unknown fields can't leak by accident. Home paths get best-effort redaction client-side before the URL is built. Submission is a prefilled `github.com/.../issues/new` URL only (see also `community.py`'s submit-url endpoint), so the user reviews the literal body on GitHub before anything is posted — the app itself never POSTs anywhere.
